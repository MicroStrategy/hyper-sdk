const status = (message, type) => {
  ...
  status.getElementsByTagName('p')[0].innerText = message;
...
const loadMainJs = (server) =>
  new Promise((resolve, reject) => {
    ...
    script.src = `${server}static/hyper/sdk/js/mstr_hyper.bundle.js`;
...
const init = async () => {
  ...
  return mstrHyper.start({
    server,
    auth: {
      autoLogin: s.autoLogin() !== 'false',
      authMode,
      username: username || null,
      password: s.password() || (username ? '' : null),
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  init()
    .then(() => {
      status('MicroStrategy HyperIntelligence is initialized.', 'success');
    })
    .catch((error) => {
      status(
